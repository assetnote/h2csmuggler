...
type Transport struct {
	...
	// This has been overriden to allow h2c support. since its not really
	// used anywhere else
	AllowHTTP bool
...
type ClientConn struct {
	...
	allowHTTP bool       // set by transport. used to determine whether the first response can be async read
	frmu      sync.Mutex // guards first read
	firstRead bool       // whether the first response for a pre-upgrade conn has been read
	...
	nextStreamID    uint32
	// is only updated on reciept of new headers (indicating a new request)
...
func (t *Transport) dialClientConn(addr string, singleUse bool) (*ClientConn, error) {
	...
	return t.newClientConn(tconn, nil, singleUse)
}
...
func (t *Transport) NewClientConn(c net.Conn) (*ClientConn, error) {
	return t.newClientConn(c, nil, t.disableKeepAlives())
}
...
func (u UnexpectedStatusCodeError) Error() string {
	return fmt.Sprintf("Unexpected status code: %v", u.Code)
}
...
func (t *Transport) H2CUpgradeRequest(req *http.Request, c net.Conn) (*ClientConn, *http.Response, error) {
	...
	if err != nil {
		return nil, nil, xerrors.Wrap(err, "failed to dump http body")
	}
...
func (t *Transport) newClientConn(c net.Conn, initialRequest *http.Request, singleUse bool) (*ClientConn, error) {
	cc := &ClientConn{
		t:                     t,
		tconn:                 c,
...
func (cc *ClientConn) roundTrip(req *http.Request) (res *http.Response, gotErrAfterReqBodyWrite bool, err error) {
	...
	cc.vlogf("writing headers %+v to stream: %+v", hdrs, cs.ID)
...
var ErrAllowHTTPNotEnabled = fmt.Errorf("Unable to read first response. Allow HTTP not enabled.")
var ErrAlreadyRead = fmt.Errorf("First read already performed.")
...
func (rl *clientConnReadLoop) run() error {
	cc := rl.cc
	cc.vlogf("http2: read loop running")
...
func (rl *clientConnReadLoop) processHeaders(f *MetaHeadersFrame) error {
	cc := rl.cc
	cc.vlogf("debug http2: processing headers !")
	cs := cc.streamByID(f.StreamID, false)
	cc.vlogf("debug http2: checking if stream exists :%+v", cs)
	if cs == nil {
	...
	cc.vlogf("debug http2: we past the first byte")
	if !cs.pastHeaders {
		cs.pastHeaders = true
	} else {
	...
	cc.vlogf("debug http2: handling response now !")
	...
	cc.vlogf("debug http2: sending resp on stream: %v", f.StreamID)
...
func (rl *clientConnReadLoop) processData(f *DataFrame) error {
	...
	if cs == nil {
		...
		if f.StreamID >= neverSent {
			...
			cc.logf("http2: Transport received unsolicited DATA frame; closing connection; got %v expecting < %v", f.StreamID, neverSent)
			return ConnectionError(ErrCodeProtocol)
		}
